import pygame as pg
import sys
from scripts.entities import EntityPhysx
...
class Game: 
    def __init__(self): # метод-конструктор, где мы выполняем инициализационные шаги, базовые функции
        ...
        self.player = EntityPhysx(self, 'player', (50, 50), (15, 15))
    ...
    def run(self):
        while True: # бесконечный цикл чтобы игра не закрывалась
            ...
            self.scroll[0] += (self.player.rect().centerx - self.display.get_width() / 2 - self.scroll[0]) / 18
            # сдвиг камеры по X = текущая позиция игрока - "позиция" дисплея (делённая на два иначе камера улетит) - где мы уже находимя
            self.scroll[1] += (self.player.rect().centery - self.display.get_height() / 2 - self.scroll[1]) / 18
            ...
            self.clock.tick(60) # устанавливаем фреймрэйт в 60 кадров/c
